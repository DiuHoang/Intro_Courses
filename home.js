...
function displayDetail(arg) {
    ...
    btnJoin.onclick = function(){
        return function(){
            ...
            var objArr = {img: prodArr[arg].image, name: prodArr[arg].name, price: prodArr[arg].price + 'đ'};
            listArr.push(objArr);
            localStorage.setItem("localProduct",JSON.stringify(listArr));
            listArr = localProduct;
            console.log(listArr);
        }
    }(arg)
...
function list(){
    ...
    var getProduct = JSON.parse(localStorage.getItem('localProduct'));
    for(var i = 0; i < listArr.length; i++){
        var divList = document.createElement('div');
        var imgList = document.createElement('img');
        imgList.src = listArr[i].img;
